...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstLine = x1 - x2;
  const secondLine = y1 - y2;
  return Math.sqrt(firstLine ** 2 + secondLine ** 2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosA = summ / (lengthA * lengthB);
  const Limited = Math.max(-1, Math.min(1, cosA));
  return Math.acos(Limited);
}
...
function getSumOfNumbers(x1, x2, x3) {
  const arr = [x1, x2, x3];
  return arr.reduce((acc, el) => acc + el).toFixed(1);
}
